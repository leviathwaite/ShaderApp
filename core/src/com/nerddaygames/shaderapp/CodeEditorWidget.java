...
public class CodeEditorWidget extends Widget
{
    ...
    public CodeEditorWidget(Skin skin) {
    ...
    private void initializeKeyboard() {
        ...
        addListener(inputListener = new ClickListener() {
            ...
            public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
                ...
                if (stage != null) stage.setKeyboardFocus(CodeEditorWidget.this);
                //Core.shared().setCurrentCursor(Core.CURSOR_TEXT);
                return true;
            }
    ...
    protected boolean onKeyTyped(InputEvent event, char character) {
        ...
        if (stage != null && stage.getKeyboardFocus() == this) {
            if (character == TAB) {
                ...
                // TODO is there a better or replacement for getFont().containsCharacter?
            // } else if (getFont().containsCharacter(character)) {
            } else if (getFont().toString().contains(String.valueOf(character))) {
    ...
    public int xToCol(float x) {
        // TODO is this the updated method?
        int c = (int) Math.floor((x - gutterWidth() - GUTTER_PADDING) / getFont().getSpaceXadvance());
    ...
    public void draw(Batch renderBatch, float parentAlpha) {
        super.draw(renderBatch, parentAlpha);
        super.draw(renderBatch, parentAlpha);
        Stage stage     = getStage();
    ...
    shape.end();*/
        ...
        if (caret.haveSelection()) {
            ...
            if (cursorRowStart == cursorRowEnd) {
                shape.rect(sx + gutterWidth() + ((cursorColStart+1) * getFont().getSpaceXadvance()) - xOffset,
                // shape.rect(sx + gutterWidth() + ((cursorColStart+1) * getFont().getSpaceWidth()) - xOffset,
                        ...
                        ((cursorColEnd - cursorColStart) * getFont().getSpaceXadvance()),
                ...
                shape.rect(sx + gutterWidth() + GUTTER_PADDING + cursorColStart * font.getSpaceXadvance(),
                // shape.rect(sx + gutterWidth() + GUTTER_PADDING + cursorColStart * font.getSpaceWidth(),
                        ...
                        width - gutterWidth() - GUTTER_PADDING - cursorColStart * font.getSpaceXadvance(),
                ...
                shape.rect(sx + gutterWidth() + GUTTER_PADDING,
                        ...
                        (cursorColEnd - caret.getColScrollPosition()) * font.getSpaceXadvance(),
        ...
        for (int y = 0; y < toLine - fromLine; y++) {
            ...
            for (int x = 0; x < line.size(); x++) {
                ...
                glyphLayout.setText(font, elem.text);
        ...
        if (focused && !disabled) {
            ...
            if (cursorOn && cursorPatch != null) {
                cursorPatch.draw(renderBatch, sx + gutterWidth() + GUTTER_PADDING + ((caret.getCol()) * getFont().getSpaceXadvance()) - xOffset, (sy + height) - (caret.getRow() - caret.getRowScrollPosition() + 1) * getLineHeight(), cursorPatch.getMinWidth(), getLineHeight());
            }
        }
    ...
    private BitmapFont getFont() {
        // TODO did this work? Probably need to load differnetly
        BitmapFont font = new BitmapFont(Gdx.files.internal("CURRIER_NEW"));
        return font;
